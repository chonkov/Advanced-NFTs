...
contract Staking is Ownable2StepUpgradeable, IERC721Receiver {
    ...
    function claimRewards(uint256 _tokenId) external {
        ...
        RewardTokenUpgradeable(_rewardToken).mint(_msgSender(), _calculatedRewards);
    ...
    function withdraw(uint256 _tokenId) external {
        ...
        RewardTokenUpgradeable(rewardToken).mint(_msgSender(), _calculatedRewards);
    ...
    function acceptOwnership(address _rewardToken) external onlyOwner {
        RewardTokenUpgradeable(_rewardToken).acceptOwnership(); // Accept ownership of the token after first being approved

        emit AcceptedOwnership(_rewardToken);
    }

    function transferOwnership(address _rewardToken, address _newOwner) external onlyOwner {
        RewardTokenUpgradeable(_rewardToken).transferOwnership(_newOwner); // Accept ownership of the token after first being approved
    }
    ...
    function calculateRewards(uint256 _lastRewardWithdrawal) public view returns (uint256, uint256) {
        ...
        uint256 _calculatedRewards = _timeSinceLastWithdrawal / 1 days * dailyReward; // 999,000 / 100.000 = 9 * 10 ether
